...
def perform_data_clean(df1,df2):
    ...
    try:
        logger.info("perform_data_clean() is started for df_city dataframe...")
    ...
    #2 Rename the columns
        logger.info("perform_data_clean() is started for df_fact dataframe...")
    ...
    #10 Impute TRX_CNT where it is null as avg of trx_cnt for that prescriber
        ...
        logger.exception("Error in the method - spark_curr_date(). Please check the Stack Trace : {error}", error=exp)
        raise
    else:

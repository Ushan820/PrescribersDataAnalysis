...
# City Report:q21
    ...
    try:
        ...
        logger.exception("Error in the method - city_report(). Please check the Stack Trace : {error}", error=exp)
        raise
    else:
...
def top_5_Prescribers(df_fact_sel):
    ...
    try:
        ...
        logger.exception("Error in the method - top_5_Prescribers(). Please check the Stack Trace : {error}" , error=exp)
        raise
    else:
